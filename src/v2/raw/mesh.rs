...
pub struct Mesh<X: Extras> {
    ...
    pub primitives: Vec<Primitive<X>>,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct Primitive<X: Extras> {
    /// Maps attribute semantic names to the `Accessor`s containing the
    /// corresponding attribute data.
    #[serde(default)]
    pub attributes: HashMap<Semantic, Index<raw::accessor::Accessor<X>>>,
    ...
    pub indices: Option<Index<raw::accessor::Accessor<X>>>,
    
    /// The material to apply to this primitive when rendering.
    pub material: Index<raw::material::Material<X>>,
    ...
    #[serde(default)]
    pub targets: Vec<HashMap<Semantic, Index<raw::accessor::Accessor<X>>>>,
}
...
impl<X: Extras> Validate<X> for Mesh<X> {
    fn validate<W, E>(&self, root: &Root<X>, mut warn: W, mut err: E)
...
impl<X: Extras> Validate<X> for Primitive<X> {  
    fn validate<W, E>(&self, root: &Root<X>, mut warn: W, mut err: E)
        where W: FnMut(&str, &str), E: FnMut(&str, &str)
    {
        use self::Semantic::*;
